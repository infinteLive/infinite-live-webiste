  ...
  const handleAllClick = () => {
    ...
    return (
      ...
      <div className="mt-10 ml-10">
            ...
            Kami perusahaan yang berspesialisasi dalam menyatukan subsistem komponen Audio Visual menjadi satu kesatuan agar dapat menyampaikan materi konten dengan maksimal.
            </h2>
        </div>
      ...
      <div onClick={handleAllClick} className="w-[200px] h-[150px] mx-auto bg-white shadow-lg rounded-xl overflow-hidden my-4 relative">
      <h2 className="text-white font-bold text-xl mb-2 absolute top-28 left-4">ALL</h2>
    ...
    {categories.map(category => {
      ...
      return (
    <div key={category}  className="w-[200px] h-[150px] mx-auto bg-white shadow-lg rounded-xl overflow-hidden my-4 relative">
      <h2 className="text-gray-900 font-bold text-xl mb-2 absolute top-28 left-4">{category}</h2>
      ...
      {selectedCategory && subCategories[selectedCategory] && (
        <div className='flex justify-center items-center mt-12'>
             <div className=" justify-center items-center flex gap-10 mb-3">
          {subCategories[selectedCategory].map(subCategory => (
            <button
              key={subCategory}
              className={`bg-zinc-200 hover:bg-orange-500  text-black hover:text-white  font-bold py-2 px-4 rounded ${selectedSubCategory === subCategory ? 'bg-zinc-200 text-black' : 'bg-orange-500 text-black'}`}
...
{Object.keys(products).length > 0 && (
  ...
  {selectedSubCategory ? (
        <h1 className='text-white text-2xl font-bold ml-20 mb-5'>{selectedSubCategory}</h1>
      ) : (
        <h1 className='text-white text-2xl font-bold ml-20 mb-5'>Products</h1>
      )}
</div>
  <div className='grid grid-cols-4'>
      ...
      {Object.keys(products).filter(subCategory => (
        ...
        products[subCategory].map(product => (
          ...
          <div className="w-[150px] h-[150px] rounded-xl  shadow-lg mx-auto bg-zinc-300 border-2 border-black">
          ...
          <div className="mb-10">
              ...
              <button onClick={() => navigate('/detailFixed')} className="bg-zinc-300 hover:bg-zinc-400 text-black font-bold py-1 px-8 rounded-lg"> Detail</button>
          </div>
      </div>
        ))
      ))}
