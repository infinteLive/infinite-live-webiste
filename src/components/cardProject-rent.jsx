...
export default function CardProjectRent() {
  ...
  const { dataRentProject } = useSelector((state) => state.dataRentProject);
  const { dataRentProjectCategory } = useSelector(
    (state) => state.dataRentProjectCategory
  );
  ...
  useEffect(() => {
    dispatch(fetchRentProject());
    dispatch(fetchRentProjectCategory());
    dispatch(fetchTotalPage("rentsProject"));
  }, []);

  const handlePaginationRentProject = (data) => {
    let currentPage = data.selected + 1;
    dispatch(fetchRentProject(currentPage));
  };
      ...
      <div className="container xl:w-[85%]">
        ...
        <div className="flex justify-center items-center mt-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 min-[320px]:p-5 sm:p-0">
            ...
            {dataRentProjectCategory.length > 0 ? (
              dataRentProjectCategory.map((item) => {
                return (
                  <button
                    key={item.id}
        ...
        <div className="flex items-center flex-col  ">
          ...
          <div className="grid xl:grid-cols-4 md:grid-cols-3 min-[320px]:grid-cols-2  gap-0 justify-between w-full">
            {dataRentProject.length > 0 ? (
              dataRentProject
                .filter((product) => {
                  if (selectedCategory) {
                    return product?.categoryProject?.name === selectedCategory;
                  }
                ...
                .map((item) => {
                  return (
                    <div
                      key={item.id}
                      onClick={() => navigate(`/detailProject-rent/${item.id}`)}
                    ...
                    >
                      ...
                      <div className="mt-0">
                        <h2 className="min-[320px]:text-lg text-center sm:text-2xl line-clamp-1 font-normal md:mb-0 lg:mb-2 text-white md:mt-3 lg:mt-2">
                          {item?.name}
                        </h2>
                        <h4
                          className="min-[320px]:text-base sm:text-[16px] font-light text-white text-center line-clamp-2"
